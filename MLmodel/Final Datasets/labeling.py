...
sentiment_dataset = pd.read_csv("Training.csv")
...
# Step 10: Preprocess the new dataset (similar to the sentiment dataset)
# Handle NaN values by filling them with an empty string
new_dataset['comment_1'] = new_dataset['comment_1'].fillna("")
new_dataset['comment_2'] = new_dataset['comment_2'].fillna("")
new_dataset['comment_3'] = new_dataset['comment_3'].fillna("")
new_dataset['comment_4'] = new_dataset['comment_4'].fillna("")
new_dataset['comment_5'] = new_dataset['comment_5'].fillna("")
# Step 12: Predict sentiment labels for comments
for i in range(1, 6):  # Loop through comment columns (comment_1, comment_2, comment_3)
    comment_col = f'comment_{i}'
    category_col = f'category_{i}'
    new_dataset[category_col] = model.predict(vectorizer.transform(new_dataset[comment_col]))
